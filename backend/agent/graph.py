...
from .utils import _get_text_from_message, _iso, _normalize_tool_result
...
async def get_graph(checkpointer):
    # MCP Client Set Up
    ...
    async def organizer(state: AgentState) -> AgentState:
        ...
        return {
            ...
            "proposed_events": validated_response.proposed_events
        }
    ...
    async def save_tool_results(state: AgentState) -> AgentState:
        ...
        raw_tool_result = _get_text_from_message(last_tool_message)
    ...
    async def commit_proposed_events(state: AgentState) -> AgentState:
        ...
        for idx, item in enumerate(proposals):
            ...
            if isinstance(item, createSeriesWithEvents) or (isinstance(item, dict) and "recurrence" in item and "event" in item):
                ...
                event = item.model_dump() if hasattr(item, "model_dump") else item
    ...
    app_graph = graph.compile(interrupt_before=['human_feedback'], checkpointer=checkpointer)
